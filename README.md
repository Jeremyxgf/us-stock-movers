...
| `stock.html` | 个股详情页（点击榜单公司名新标签页打开）：12 个月换手衰减筹码分布 + 机构/散户分层推断（量价行为 + 5 分钟大单集中度代理），外加 **5 分钟分布双图**：波动率热力图（蓝）与换手率热力图（橙），各带时段剖面（交易日 × 时段，可切近5/10/22日；点行下钻，含同时段中位与 P25–P75 分位带）。 |
...
| `cn-stock.html` | 个股详情：腾讯前复权日 K + 新浪快照流通市值算流通股，机构/散户推断（可另参考龙虎榜/北向）；同样带 **5 分钟波动率 + 换手率双热力图**（午休时段自动压缩并画分隔线；涨跌停封板期在波动率图上呈现为一整片浅色「死区」）。 |
| `data/intraday/<代码>.json`<br>`data/cn/intraday/<代码>.json` | 每股 22 日逐 5 分钟明细（`r` = \|对数收益\|×10000 的整数基点，`null` = 当日首根；`t` = 成交量占流通盘的百万分之几，即 5 分钟换手率）。由 `fetch_intraday.py` / `cn_fetch_intraday.py` 顺带写出——bar 本就在算 RV 时读进内存，只是不再丢弃，**零额外请求**。个股页热力图只取自己那一个文件（约 6 KB）。 |
...
- **日内已实现波动率**（`intraday.html` / `stock.html` 热力图）：`RV_d = √(Σ ln(cᵢ/cᵢ₋₁)²)`，5 分钟收盘价、仅日内相邻（不含隔夜跳空）。
  ...
  个股页热力图读同一条管线写出的 `data/intraday/<代码>.json`，**一行的各格平方和开根 == 该股当天的 RV**（已对 500 只逐日校验，零偏差）。
  **5 分钟换手率** = 该 bar 成交量 ÷ 流通股，存成「百万分之几」的整数（3–4 位，比直接存成交量的 6–8 位省得多）。
  A 股 5 分钟量合计恰等于日成交量，故逐格相加 == 当日换手率（已与快照 `turnover` 字段对账，中位比值 1.000）；
  美股 5 分钟 bar 只覆盖常规时段，逐格相加约为官方日换手率的 81%（差额来自盘前盘后与非交易所成交），跨时段比较不受影响。
  覆盖范围 = 最活跃 500 池（与「5 分钟大单集中度」一致），池外股票该图显示不可用。
