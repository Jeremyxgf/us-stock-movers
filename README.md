...
| `stock.html` | 个股详情页（点击榜单公司名新标签页打开）：12 个月换手衰减筹码分布 + 机构/散户分层推断（量价行为 + 5 分钟大单集中度代理），外加 **5 分钟波动率热力图 + 时段剖面**（交易日 × 时段，可切近5/10/22日；点行下钻到当日剖面，含同时段中位与 P25–P75 分位带）。 |
...
| `data/intraday/<代码>.json`<br>`data/cn/intraday/<代码>.json` | 每股 22 日逐 5 分钟明细（`r` = \|对数收益\|×10000 的整数基点，`null` = 当日首根）。由 `fetch_intraday.py` / `cn_fetch_intraday.py` 顺带写出——bar 本就在算 RV 时读进内存，只是不再丢弃，**零额外请求**。个股页热力图只取自己那一个文件（约 6 KB）。 |
...
- **日内已实现波动率**（`intraday.html` / `stock.html` 热力图）：`RV_d = √(Σ ln(cᵢ/cᵢ₋₁)²)`，5 分钟收盘价、仅日内相邻（不含隔夜跳空）。
  ...
  逐 bar 成交量不落盘：实测会让明细体积涨 3 倍，而热力图只需要波动率；只保留每日总量 `vt` 供 tooltip 显示。
